...
export default function ReceptionLinkManager({
    ...
    // Form Input states (Transient)
    const [inputStart, setInputStart] = useState(formatForDateTimeLocal(initialStart));
    const [inputEnd, setInputEnd] = useState(formatForDateTimeLocal(initialEnd));
    ...
    const handleConfirmAction = async () => {
        ...
        try {
            if (currentModal === 'TOGGLE_STATUS') {
                ...
                await Promise.all([
                    updateReceptionStart(productionId, inputStart),
                    updateReceptionStatus(productionId, 'CLOSED')
                ]);

                setManualStatus('CLOSED');
                setConfirmedStart(inputStart ? new Date(inputStart) : null);
                ...
                const formData = new FormData();
                formData.append('receptionEnd', inputEnd);
                ...
                setManualStatus('CLOSED');
                setConfirmedEnd(inputEnd ? new Date(inputEnd) : null);
                    ...
                    <div className="form-group" style={{
                        ...
                        <div style={{ display: 'grid', gap: '1rem' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                ...
                                />
                                <span className="text-muted" style={{ fontSize: '0.85rem' }}>から受付開始</span>
                            </div>
                    ...
                    <div className="form-group" style={{ padding: '1.5rem', border: '1px solid var(--card-border)', borderRadius: '8px', background: '#fcfcfc' }}>
                        ...
                        <div style={{ display: 'grid', gap: '1.25rem' }}>
                            <label style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', cursor: 'pointer' }}>
                                ...
                                {endMode === 'MANUAL' && (
                                    <input type="datetime-local" className="input" value={inputEnd} onChange={(e) => setInputEnd(e.target.value)} style={{ margin: '0 0 0 1rem', width: 'auto' }} />
                                )}
                            </label>
                                    ...
                                    {endMode === 'BEFORE_PERFORMANCE' && (
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginLeft: '0.5rem' }}>
                                            <input
                                                ...
                                                value={endHours}
                                                min="0"
                                                onChange={(e) => setEndHours(parseInt(e.target.value || '0', 10))}
                                                style={{ width: '60px', margin: 0, padding: '0.4rem' }}
                                            />
                                            <span style={{ fontSize: '0.85rem' }}>時間</span>
                                            <input
                                                type="number"
                                                className="input"
                                                ...
                                                min="0"
                                                max="59"
                                                onChange={(e) => setEndMinutesPart(parseInt(e.target.value || '0', 10))}
                                                style={{ width: '60px', margin: 0, padding: '0.4rem' }}
                                            />
                                            <span style={{ fontSize: '0.85rem' }}>分前</span>
                                        </div>
                                    )}
                                </div>
                            </label>
