...
export default function TicketTypeManager({ productionId, ticketTypes }: Props) {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            await deleteTicketType(idToDelete, productionId);
        } catch (error: any) {
    ...
    return (
        <section>
                        ...
                        <li key={ticket.id} style={{ padding: '0.75rem 0', borderBottom: '1px solid var(--card-border)' }}>
                            {editingId === ticket.id ? (
                                <form action={async (formData) => {
                                    setIsProcessing(true);
                                    try {
                                        await updateTicketType(ticket.id, formData);
                                        setEditingId(null);
                                    } catch (error: any) {
            ...
            <div className="card">
                ...
                <form action={addTicketType} style={{ display: 'flex', flexWrap: 'wrap', gap: '1.5rem', alignItems: 'flex-end' }}>
                    <input type="hidden" name="productionId" value={productionId} />
