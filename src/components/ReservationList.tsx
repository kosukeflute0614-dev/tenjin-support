                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                    <thead style={{ backgroundColor: 'var(--secondary)', color: 'var(--text-muted)' }}>
                        <tr>
                            ...
                            <th style={{ padding: '1rem' }}>支払</th>
                            <th style={{ padding: '1rem' }}>操作</th>
                        </tr>
                    </thead>
                    <tbody>
                        {filteredReservations.map((res) => {
                            const totalCount = res.tickets.reduce((sum: number, t: any) => sum + t.count, 0);
                                ...
                                <tr key={res.id} style={{
                                    ...
                                    <td style={{ padding: '1rem' }}>
                                        <div style={{ fontSize: '0.9rem' }}>
                                            {res.performance.production.title}
                                        </div>
                                    ...
                                    <td style={{ padding: '1rem' }}>
                                        <span style={{
                                            ...
                                            borderRadius: '4px',
                                            backgroundColor: res.status === 'CONFIRMED' ? 'rgba(76, 217, 100, 0.2)' : isCanceled ? 'rgba(150, 150, 150, 0.2)' : 'rgba(255, 75, 75, 0.2)',
                                            color: res.status === 'CONFIRMED' ? 'var(--success)' : isCanceled ? '#999' : 'var(--accent)',
                                            fontSize: '0.8rem'
                                        }}>
                                    ...
                                    <td style={{ padding: '1rem' }}>
                                        <span style={{
                                            color: res.paymentStatus === 'PAID' ? 'var(--success)' : 'var(--text-muted)'
                                        }}>
                                            {PAYMENT_STATUS_LABELS[res.paymentStatus] || res.paymentStatus}
                                        </span>
                                    </td>
                                    <td style={{ padding: '1rem' }}>
                    ...
                    <div style={{
                        ...
                        <div className="card" style={{
                            ...
                            <div style={{
                                ...
                                <div><strong>合計枚数:</strong> {cancellingReservation.tickets.reduce((sum: number, t: any) => sum + t.count, 0)} 枚</div>
                                <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '0.5rem' }}>
                                    {cancellingReservation.tickets.map((t: any) => `${t.ticketType.name} x ${t.count}`).join(', ')}
                                </div>
                            </div>
