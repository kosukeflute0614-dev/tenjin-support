...
export default async function BookSuccessPage({ params }: { params: Promise<{ productionId: string }> }) {
    const { productionId } = await params;
    const productionRef = doc(db, "productions", productionId);
    const productionSnap = await getDoc(productionRef);
    const production = productionSnap.exists() ? productionSnap.data() : null;
        ...
        <div className="container" style={{ maxWidth: '600px', textAlign: 'center', paddingTop: '5rem' }}>
            <div className="card" style={{ padding: '4rem 2rem', borderTop: '4px solid var(--primary)' }}>
                ...
                <p style={{ color: 'var(--text-muted)', lineHeight: '1.8', marginBottom: '2rem' }}>
                    「{production?.title}」へのご予約を承りました。<br />
